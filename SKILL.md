...
所有命令从 `{baseDir}` 目录运行，先activate该目录的venv
...
默认使用./feed_brief.sh脚本，获取评论在使用feed脚本。
```bash
cd {baseDir}

# 使用上一步搜索结果中的 id 和 xsec_token。如果用户要求包含图片，就加上--image，根据返回的image链接保存图片到workspace/tmp目录下，在回答后把图片发送给用户
./feed_brief.sh <feed_id> <xsec_token> [--image]
